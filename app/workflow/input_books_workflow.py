...
from .steps.extract_books_data import ExtractBooksDataStep
...
class InputBooksWorkflow:
    ...
    def __init__(self, model: OpenAIService, mcp: MCPManager):
        self.extract_books_data = ExtractBooksDataStep(model, mcp)
        self.insert_books_step = InsertBooksStep(model, mcp)

    async def run(self):
        # Step 1: Extract books from PDF receipt
        books: list[Book] = await self.extract_books_data.run()
